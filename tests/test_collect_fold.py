...
def test_varying_number_of_columns(num_cols: int, extra_args: Sequence[float]):
    ...
    assert collect_fold(
        df, add_columns, 7, extra_args, [str(col) for col in range(1, num_cols + 1)]
...
def test_lazy_and_not_lazy():
    ...
    for test_df in [df, df.lazy()]:
        assert collect_fold(test_df, add_columns, 0.5) == 83.5
...
def test_nulls_filtered_out():
    ...
    assert (
        collect_fold(df, add_columns, 0.5, column_names=["a", "b"])
        == 0.5 + 1 + 30 + 3 + 100
    )
...
def test_accumulator_type_casting():
    ...
    result = collect_fold(df, to_f32, np.int64(10))
    assert result == 13.75
...
def test_compiled_function_caching():
    ...
    with measure_cpu_time() as elapsed:
        assert collect_fold(df, multiply, np.uint64(2)) == 6
